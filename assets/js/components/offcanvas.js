 ...
 * - offcanvas:show   — before opening (transition starts)
 * - offcanvas:shown  — after opening (transition ends, focus moved)
 * - offcanvas:hide   — before closing (transition starts)
 * - offcanvas:hidden — after closing (transition ends, focus restored)
 *
 * A11y:
...
class Offcanvas {
  ...
  show() {
    ...
    this.el.dispatchEvent(new CustomEvent('offcanvas:show', { bubbles: true }));
    ...
    this._afterTransition(() => {
      ...
      focusFirst(this.el);
      this.el.dispatchEvent(new CustomEvent('offcanvas:shown', { bubbles: true }));
    });
  }
  ...
  hide() {
    ...
    this.el.dispatchEvent(new CustomEvent('offcanvas:hide', { bubbles: true }));
    ...
    this._afterTransition(() => {
      ...
      this.el.dispatchEvent(new CustomEvent('offcanvas:hidden', { bubbles: true }));
    });
  }

  static getInstance(el) {
    return el._offcanvasInstance ?? null;
  }
}
...
document.querySelectorAll('.js-offcanvas-toggle').forEach((trigger) => {
  ...
  target._offcanvasInstance = instance;
